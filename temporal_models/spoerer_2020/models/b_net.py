...
def b_layer(x, filters, kernel, layer_num, pooling=True):
    ...
    if pooling: # with exception of the first layer
    ...
    data_format = tf.keras.backend.image_data_format()                          # returns the default image data format convention.
    norm_axis = -1 if data_format == 'channels_last' else -3                    # output dimensions (either at index 0 if channels_last, or at index 2 if channels_first)
