...
def main():
    ...
    if layernum != 1:
        ...
        ax.plot(t, avg_act_array[0]/np.amax(avg_act_array[0]), label='act layer '+str(layer), color=clls[0])
        ax.plot(t, avg_sup_array[0]/np.amax(avg_sup_array[0]), label='sup layer '+str(layer), color=clls[0], linestyle = '--', alpha=0.5)
